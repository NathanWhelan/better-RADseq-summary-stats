...
.cli_usage <- list(
  diversity_stats = c(
    ...
    "  Run it on both: the SNP VCF gives Ho, He and the per-site values; the",
    "  haplotype VCF gives Fis, Ar and rarefied private alleles.",
    "",
    ...
    "                   population's own Sites is then read from it; right only",
    "                   if nothing was filtered after Stacks populations)",
  ...
  het_between_pops = c(
    ...
    "       Rscript het_between_pops.R --selftest",
    "",
    "  Tests whether heterozygosity differs between populations: one value per",
    "  individual, and a test that counts both which individuals and which RAD",
    "  loci were sampled. Needs a VCF: per-individual heterozygosity cannot be",
    "  recovered from populations.sumstats.tsv.",
    "",
