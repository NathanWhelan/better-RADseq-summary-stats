...
Four changes to an ordinary Stacks run drive all of it. In practice, changes
like these can move reported F<sub>IS</sub> substantially — enough, in some
datasets, to flip a borderline significant difference between populations
into a non-significant one — so getting them right matters more than it
looks.
...
Two flags and two scripts. Three commands total. Everything else in this
document is diagnosis, and none of it changes these numbers.
...
the right source for. Do not mix them up — see "Which file for which
statistic" below.

The rest of this document is the reasoning behind those commands: why these
particular flags and estimators, what each one fixes, and what to write in a
...
| HWE filtering | a heterozygote deficit is the signal, not the noise. Filtering on HWE removed roughly a quarter of one dataset's F<sub>IS</sub> in testing — created entirely by the filter. |
