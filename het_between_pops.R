...
###############################################################################
#
#  het_between_pops.R -- does mean heterozygosity differ between populations?
...
if (length(.a0) >= 1 && .a0[1] == "--selftest") {
  ...
  sim <- function(f_sd) {
    p <- runif(L, 0.05, 0.95)
    F1 <- pmin(pmax(rnorm(n1, .1, f_sd), 0), .9); F2 <- pmin(pmax(rnorm(n2, .1, f_sd), 0), .9)
    ih <- function(Fv, n) vapply(seq_len(n), function(j)
            mean(runif(L) < 2*p*(1-p)*(1-Fv[j])), numeric(1))
    list(a = ih(F1, n1), b = ih(F2, n2))
  }
  cat(sprintf("  %-24s %10s %10s\n", "individual F variation", "Welch t", "Wilcoxon"))
  for (sd in c(0.02, 0.10, 0.25)) {
    r <- replicate(nsim, { x <- sim(sd)
      c(t.test(x$a, x$b)$p.value,
        suppressWarnings(wilcox.test(x$a, x$b))$p.value) })
    cat(sprintf("  SD(F) = %.2f %-13s %9.1f%% %9.1f%%\n", sd, "",
                100 * mean(r[1, ] < 0.05), 100 * mean(r[2, ] < 0.05)))
  }
  ...
  for (sd in c(0.00, 0.10)) {
    rej <- replicate(120, {
      p <- runif(L, .05, .95)
      F1 <- pmin(pmax(rnorm(n1, .1, sd), 0), .9); F2 <- pmin(pmax(rnorm(n2, .1, sd), 0), .9)
      mk <- function(Fv, n) { h <- matrix(FALSE, L, n)
        for (j in seq_len(n)) h[, j] <- runif(L) < 2*p*(1-p)*(1-Fv[j]); h }
      h1 <- mk(F1, n1); h2 <- mk(F2, n2)
      a <- rowMeans(h1); b <- rowMeans(h2)
      bo <- replicate(300, { i <- sample.int(L, L, TRUE); mean(a[i]) - mean(b[i]) })
      !(0 >= quantile(bo, .025) && 0 <= quantile(bo, .975)) })
...
min_call <- if (length(args) >= 3) as.numeric(args[3]) else 0.9
...
src <- if (is.na(.self)) NA_character_ else file.path(dirname(.self), "haps_common.R")
if (is.na(src) || !file.exists(src)) {
  ## Could not resolve this script's own directory (e.g. invoked via
  ## `R -e 'source(...)'` rather than `Rscript path/to/het_between_pops.R`).
  ...
  warning("Could not resolve this script's own directory; looking for ",
          "haps_common.R in the current working directory (", getwd(),
          ") instead. Run via `Rscript /path/to/het_between_pops.R ...` ",
          "to avoid this.")
  src <- "haps_common.R"
}
if (!file.exists(src)) stop("haps_common.R must sit alongside this script.")
source(src)
...
## script still exits 0. A failed library is exactly the case this script is
## meant to detect, so it must not be the case it fails quietly on. Drop such
## individuals loudly and refuse if that leaves a population too small.
.thin <- ncall < 50
if (any(.thin)) {
...
pop_het <- setNames(vector("list", r), names(pops))
for (p in names(pops)) {
  lp <- keep_pop[, p]
...
summ <- do.call(rbind, lapply(names(pops), function(pn) {
  ...
  data.frame(population = pn, n = length(v), n_loci = sum(keep_pop[, pn]),
             mean_het = round(mean(v), 4), sd = round(sd(v), 4),
             se = round(sd(v) / sqrt(length(v)), 4),
...
for (i in seq_len(r - 1)) for (j in (i + 1):r) {
  ...
  wt <- suppressWarnings(try(wilcox.test(a, b), silent = TRUE))
...
tab$p_welch_BH  <- p.adjust(tab$p_welch,  method = "BH")
tab$p_wilcox_BH <- p.adjust(tab$p_wilcox, method = "BH")
out <- tab
...
if (npair <= 25) print(out, row.names = FALSE) else {
  ...
  print(out[head(o, 25), ], row.names = FALSE)
}
...
write.table(ind, file.path(outdir, "individual_heterozygosity.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)
write.table(out, file.path(outdir, "het_between_pops_tests.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)
