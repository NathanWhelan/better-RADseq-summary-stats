...
> `diversity_core_selftest()`. A script prints the full report; in R, printing
> a result shows its main tables and `summary(result)` shows the full report.
>
...
**No `--min-mac` or `--min-maf` in this run.** An allele-frequency filter
removes rare variants, and rare variants carry a large share of the
diversity. Stacks does not delete a SNP that fails the filter: it turns it
into a fixed site (`LocusFilter::erase_snp()`), which still counts in
`Sites`, so the loss shows up in Stacks' own all-positions `Pi`, in
`He_autosomal` and in `pi_allsites()` alike. Under a neutral site-frequency
spectrum, a site whose derived allele is carried by *i* of *N* gene copies
contributes in proportion to *N* − *i*, so sites with a minor allele count of
2 or less hold about 4/(*N* − 1) of π:

| diploids in the run | share of π removed by `--min-mac 3` |
|---|---|
| 10 | 21% |
| 20 | 10% |
| 30 | 7% |

That is a bias that depends on sample size — the kind this whole document
argues against — and it also removes rare, often population-private,
heterozygous sites before `het_between_pops()`. If STRUCTURE, PCA or a
kinship estimator needs a MAC or MAF filter, make a second, filtered
`populations` run for those analyses.
...
output formats some workflows want directly). These are conventional. Set
them how you like; just set them the same way for both populations.
...
`--g=N` (required), `--nboot=N`, `--sites=N`, `--min-n=N`, `--seed=N`,
`--complete-case`. Each run prints a `TAKE FROM THIS RUN` box (in R:
`summary(result)`) naming the statistics it is the right file for, so you do
not have to remember the split.
...
**Free diagnostic — run it:** F<sub>IS</sub> from the two files should agree
closely. If it does not, that is informative. The usual suspects:

- Sequencing error creating spurious rare haplotypes. This inflates haplotype
  H<sub>o</sub> and pushes haps F<sub>IS</sub> down.
- Missing data that is not random with respect to genotype. Stacks writes a
  whole haplotype genotype as missing (`./.`) if *any* SNP on the tag is
  uncalled in that individual (`export_formats.cc`, Stacks 2.68). A true
  heterozygote needs more reads to be called than a homozygote, so tags with
  several SNPs are more likely to go missing in heterozygous individuals.
  That would push haps H<sub>o</sub> down and haps F<sub>IS</sub> up, most in
  low-coverage libraries.

Run both files and report the check.
...
that difference on the one axis this conversion is about. The numerator is
per population too: each population's `He` is multiplied by the number of
variant records at which it has a genotyped individual (`variant_records` in
the output), which is what Stacks pairs with its `Sites`. `sites` accepts:
...
- a path to `populations.sumstats_summary.tsv` itself — each population's own
  `Sites` and `Variant_Sites` (the All-positions block) are read
  automatically, by name, via `read_sumstats_summary()`. This is the safest
  form. If records were removed in R after reading (`filter_*()`), those
  sites are still in `Sites`, and the file's unfiltered `Variant_Sites` keeps
  the scaling right. A warning says when the VCF and the file disagree.
...
reasons in "Formulas" below. The only columns this package's own functions
ever read from that file are `Sites` and `Variant_Sites`, plain tallies
...
instead. Petit & Pons (1998) repeat it; Neff & Fraser (2010) wrote software for both. Most
papers ignore it.
...
The script's autosomal figure, for each population, is `He_autosomal` = (mean
`He` over the loci actually used) × *n*<sub>var</sub> / *S*.

- *S* is that population's `Sites`. Stacks counts a site there only if the
  population has at least one genotyped individual at it
  (`SumStatsSummary::accumulate()`, Stacks 2.68).
- *n*<sub>var</sub> must be counted the same way: the variant sites at which
  that population has at least one genotyped individual. That is Stacks'
  `Variant_Sites` in the same block, and the `variant_records` column of the
  output.

*n*<sub>var</sub> is not every record in the VCF. With three or more
populations and `-p` below their number, Stacks blanks a population at the
sites where it fails `-r` and keeps the site for the others. Those records
are not in that population's *S*, and scaling by all records would inflate
its `He_autosomal` — by 1/0.7 if it is absent from 30% of records.

Nor is *n*<sub>var</sub> just the used subset: scaling by the subset would
...
Neff, B.D. & Fraser, B.A. (2010) A program to compare genetic differentiation
statistics across loci using resampling of individuals and loci. *Molecular
Ecology Resources* 10: 546–550.
