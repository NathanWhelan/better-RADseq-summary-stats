...
> values are scaled, for each population, by all variant sites at which that
> population has a genotyped individual (Stacks' `Variant_Sites`, the count
> Stacks pairs with its `Sites`), not only by the records that cleared the
> bar, since the used subset is a sample of those sites rather than the whole
> of them.
>
